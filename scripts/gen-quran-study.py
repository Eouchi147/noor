...
Sources (all under build/):
  ...
  study-light.json    72 surahs, short records
Together: 114, no gaps, no duplicates.
...
def check(records, table):
    ...
    for rec in records.values():
        ...
        for ref in refs:
            ...
            if not m:
                problems.append("surah %d: passage ref %r is not surah:ayah" % (n, ref))
                continue
            if int(m.group(1)) != n or not 1 <= int(m.group(2)) <= info["count"]:
        ...
        if virtue and virtue.get("level") not in LEVELS:
            problems.append("surah %d: virtue level %r is neither sunnah nor debated" % (n, virtue.get("level")))
    return problems
...
def build():
    ...
    lines = [
        ...
        "   Generated by scripts/gen-quran-study.py from build/study-deep-a.json,",
        "   build/study-deep-b.json and build/study-light.json. Do not edit by hand.",

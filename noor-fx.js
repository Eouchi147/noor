...
(function () {
  ...
  var box = null, input = null, body = null, openNow = false, tid = 0, from = null;
  ...
  function shell() {
    ...
    box.innerHTML = '<div class="nmr-back"></div>' +
      '<div class="nmr-p" role="dialog" aria-modal="true" aria-label="The library" tabindex="-1">' +
  ...
  /* It says aria-modal, so it behaves as one: Escape closes it, and Tab
     stays inside it. Until 16 September 2026 a Tab from the field walked
     out into the page behind the sheet, and closing dropped focus to the
     top of the document; focus now goes in when it opens and comes back to
     the door that opened it when it closes. */
  function focusable() {
    var p = box.querySelector(".nmr-p");
    return [].slice.call(p.querySelectorAll("a[href],button:not([disabled]),input:not([disabled]),[tabindex]:not([tabindex='-1'])"))
  ...
  function esckey(e) {
    ...
    if (e.key !== "Tab") return;
    var p = box.querySelector(".nmr-p"), f = focusable(), a = doc.activeElement;
    if (!f.length) { e.preventDefault(); p.focus(); return; }
    var first = f[0], last = f[f.length - 1];
    if (e.shiftKey && (a === first || a === p || !p.contains(a))) { e.preventDefault(); last.focus(); }
  ...
  function open() {
    ...
    /* focus goes in: the field where there is a pointer to type with, the
       sheet itself on a phone, where a focused field would raise the keyboard
       over the map the reader came to read */
    setTimeout(function () {
      if (!openNow) return;
      if (matchMedia("(hover:hover)").matches) input.focus();
      else box.querySelector(".nmr-p").focus({ preventScroll: true });
    }, 60);
  }
  function close() {
    if (!box) return;
    ...
    /* and back to the door that opened it */
